...
class Simple_Google_Docs_Viewer {
	...
	function the_shortcode( $atts, $content = '' ) {
		...
		if ( '' != ( $file = apply_filters( 'simple_gviewer_file_url', $file ) ) ) {
			$embed_format = '<iframe src="https://docs.google.com/viewer?url=%1$s&embedded=true&hl=%2$s" width="100%%" height="%4$d" style="border: none; max-width:' . $content_width . 'px;"></iframe>';
